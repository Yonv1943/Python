...
def mp_worker(args, pipe_exp, i):
    ...
    if if_on_policy:
        ...
        ReplayBuffer(max_len=max_memo // worker_num, if_use_per=if_per_or_gae,
                     state_dim=state_dim, action_dim=1 if if_discrete else action_dim)

    if if_vec_env:
        agent.state = env.reset_vec()
        # agent.state = [e.reset() for e in env.env_list]  # todo temp
    ...
    if if_on_policy:
        ...
        with torch.no_grad():
            while True:
                ...
                # trajectory_list = agent.explore_env(env, _target_step, reward_scale, gamma)
                buffer = agent.explore_env(env, _target_step, reward_scale, gamma)
...
def mp_learner(args, pipe_exp_list, pipe_eva):
    args.init_before_training(process_id=0)
