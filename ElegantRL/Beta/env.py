...
class PreprocessVecEnv:  # environment wrapper
    def __init__(self, env, env_num, if_print=True, data_type=torch.float32):
        self.env = gym.make(env) if isinstance(env, str) else env
        ...
        self.env_list = [PreprocessEnv(self.env_name, if_print=False) for _ in range(env_num)]
        for i in range(env_num):
            [self.env_list[i].reset() for _ in range(i)]  # random seed

    def reset_vec(self):
        states = [torch.as_tensor(env.reset(), dtype=self.data_type, device=self.device)
...
class PreprocessVecEnv0(gym.Wrapper):
    ...
    def step(self, actions) -> (torch.Tensor, torch.Tensor, torch.Tensor, dict):
        ...
        states = torch.empty((self.env_num, self.state_dim), dtype=self.data_type, device=self.device)
        rewards = torch.empty(self.env_num, dtype=self.data_type, device=self.device)
        dones = torch.empty(self.env_num, dtype=self.data_type, device=self.device)
        for i in range(self.env_num):
            state, reward, done, _ = self.env_list[i].step(actions[i])
            states[i] = torch.as_tensor(self.env_list[i].reset() if done else state)
            rewards[i] = torch.as_tensor(reward)
