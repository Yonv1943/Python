...
class AgentPPO(AgentBase):
    ...
    def explore_envs(self, env, target_step, reward_scale, gamma):
        ...
        for i in range(buf_step):
            ...
            next_state, reward, done, _ = env.step(action.tanh())
    ...
    def prepare_buffers(self, buffer):
        with torch.no_grad():  # compute reverse reward
            ...
            for j in range(env_num):
                ...
                logprobs[:, j] = self.act.get_old_logprob(actions[:, j], noises[:, j])

            pre_states = torch.as_tensor(self.state, dtype=torch.float32, device=self.device)
            pre_r_sums = 0  # todo beta0 self.cri(pre_states).detach().squeeze(1)
    ...
    def get_reward_sum_gae(self, buf_shape, buf_reward, buf_mask, buf_value, pre_r_sum) -> (torch.Tensor, torch.Tensor):
        ...
        # pre_advantage = 0  # advantage value of previous step  # wait todo
